...
const fs = require('fs');
const db = require('./models'); // ⭐ TAMBAHKAN: Import Sequelize models
...
function validateModelFiles() {
    ...
    if (missing.length > 0) {
        ...
        // ⭐ JANGAN EXIT jika model belum ada, biar backend tetap jalan
        console.warn('⚠️  ML features will be disabled until models are available');
    } else {
...
app.get('/health', (req, res) => {
    res.json({
        status: 'healthy',
        service: 'Telco Backend (with Admin API)',
        timestamp: new Date().toISOString(),
        uptime: process.uptime(),
        database: db.sequelize ? 'connected' : 'not connected',
        models: { loaded: fs.existsSync(MODEL_PATH) },
...
app.get('/', (req, res) => {
    res.json({
        ...
        endpoints: {
            ...
            ml: '/api/predict, /api/predict-save',
            health: '/health'
        }
    });
});
...
db.sequelize.sync({ alter: false })
    .then(() => {
        ...
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`\n${'='.repeat(60)}`);
            console.log(`🚀 Telco Backend running on http://localhost:${PORT}`);
            console.log(`📡 Admin API: http://localhost:${PORT}/api/admin`);
            console.log(`🤖 ML Backend: ${fs.existsSync(MODEL_PATH) ? 'ENABLED' : 'DISABLED (models not found)'}`);
    ...
    .catch(err => {
        ...
        // ⭐ Tetap jalankan server meskipun DB error
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`\n⚠️  Server running on http://localhost:${PORT} (DB connection failed)`);
        });
    });